...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d p = _x - m_center;
		p = p.array().square();
		double val = m_radius - sqrt(p(0) + p(1));
		return val*val + p(2) - (m_a*m_a);
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d temp = (_x - p.cast<double>());
		double val = temp.dot(n.cast<double>());
		return val;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		result += m_coefficents[m_numCenters]*_x[0] + m_coefficents[m_numCenters+1]*_x[1] + m_coefficents[m_numCenters+2]*_x[2];
...
#define phi(i,j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
	...
	void BuildSystem()
	{
		...
		for(int i=0; i<(2 * m_numCenters); i++){
			...
			Vector3d point = m_funcSamp.m_pos[i];
			A(i,m_numCenters) = point[0];
			A(i,m_numCenters+1) = point[1];;
			A(i,m_numCenters+2) = point[2];
